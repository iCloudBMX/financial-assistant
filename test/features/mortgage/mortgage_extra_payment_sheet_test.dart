...
void main() {
  testWidgets('recording an extra payment lowers the balance and closes',
    ...
    expect(find.byType(AccountCardPicker), findsOneWidget); // source card
    ...
    expect(
        ...
        100000000 - 2000000);
    // Drawn from the (auto-selected) card: 500M - 2M = 498M.
    final accounts = await container.read(accountsControllerProvider.future);
    expect(accounts.single.balance.minorUnits, 498000000);
  });

  testWidgets('an extra payment exceeding the card balance is blocked',
      (t) async {
    final db = AppDatabase(NativeDatabase.memory());
    addTearDown(db.close);
    final container = ProviderContainer(
    ...
    // 600M > 500M on the card.
    await t.enterText(find.byKey(const Key('extra-amount')), '600000000');
    await t.pumpAndSettle();

    expect(find.text('Kartada yetarli mablag\' yo\'q'), findsOneWidget);
    // Save is disabled: tapping records nothing (a live balance guard, not
    // just a post-hoc _save check).
    await t.tap(find.byKey(const Key('extra-save')));
    await t.pumpAndSettle();
    expect(await container.read(mortgageRepositoryProvider).payments(id),
        isEmpty);
  });
